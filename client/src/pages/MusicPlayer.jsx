...
function MusicPlayer() {
  ...
  useEffect(() => {
    ...
    const handleSongEnd = () => playNext(); // Automatically play next song when current one ends
    ...
    return () => {
      ...
      audio.removeEventListener("ended", handleSongEnd); // Clean up event listener
    };
  }, [currentSong, isPlaying, audioRef, playNext]); // Added playNext to dependencies

  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.volume = volume;
    }
  }, [volume, audioRef]); // Added audioRef to dependencies
  ...
  // If no song is loaded, display a subtle message or hide the player
  if (!currentSong) {
    return (
      <footer className="fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-gray-700 z-50 p-4 text-center text-gray-500 text-sm">
        No song currently playing. Explore and pick one!
      </footer>
    );
  }
      ...
      <div className="flex items-center justify-between px-8 py-4 text-white">
        ...
        <div className="w-1/4 flex items-center space-x-5">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 truncate w-48">{currentSong.artist}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center flex-grow mx-8">
          ...
          <div className="flex items-center w-full space-x-3 text-sm text-gray-400">
            ...
            <input
              ...
              value={currentTime}
              onChange={handleSeek}
              className="flex-1 h-2 rounded-full bg-gray-700 appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-purple-500 custom-seekbar" // Custom seekbar styling
              style={{
        ...
        <div className="w-1/4 flex items-center justify-end space-x-4">
          ...
          <input
            ...
            className="w-28 h-2 rounded-full bg-gray-700 appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-purple-500 custom-volume-slider" // Custom volume slider styling
            style={{
                background: `linear-gradient(to right, #8b5cf6 0%, #8b5cf6 ${
                  volume * 100
