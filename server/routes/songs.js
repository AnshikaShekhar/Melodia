...
const storage = new CloudinaryStorage({
  ...
  params: async (req, file) => {
    const folder = file.fieldname === "audio" ? "melodia-audio" : "melodia-images";
    return {
      folder,
      resource_type: file.fieldname === "audio" ? "video" : "image",
      allowed_formats: file.fieldname === "audio" ? ["mp3", "wav"] : ["jpg", "jpeg", "png"],
    };
  },
});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!title || !artist || !genre || !mood || !duration || !audioFile || !imageFile) {
        return res.status(400).json({ message: "All fields (title, artist, genre, mood, duration, audio, image) are required." });
      }
      ...
      if (audioFile.size > 10 * 1024 * 1024) {
        return res.status(400).json({ message: "Audio file size must be under 10MB." });
      }
      if (imageFile.size > 5 * 1024 * 1024) {
        return res.status(400).json({ message: "Image file size must be under 5MB." });
      }
